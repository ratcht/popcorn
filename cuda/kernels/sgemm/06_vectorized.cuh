#ifndef SGEMM_VECTORIZED_CUH
#define SGEMM_VECTORIZED_CUH
...
__global__ void sgemm_vectorized(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C) {
  ...
  for (int block = 0; block < K; block+=BK) {
    ...
    reinterpret_cast<float4*>(&Bs[tyB][txB * 4])[0] =
      reinterpret_cast<float4*>(&B[tyB * N + txB * 4])[0];
